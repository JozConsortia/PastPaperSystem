...
type PaperSearchResult = SearchResource & {
  fileUrl?: string;
  memoUrl?: string;
};
...
function SearchResultsView() {
  ...
  const [paperResults, setPaperResults] =
  ...
  useEffect(() => {
    apiRequest<Array<{
      ...
      .then((papers) => {
        const results = papers.map((paper): PaperSearchResult => ({
          id: `paper-${paper.id}`,
          title: paper.title,
          category: paper.educationLevel === "TVET College"
            ? "TVET"
            : paper.educationLevel as SearchResource["category"],
          type: "Paper",
          description: `${paper.phase} · Grade ${paper.grade} · ${paper.subject}. ${paper.description}`,
          keywords: [paper.title, paper.educationLevel, paper.phase, paper.grade, paper.subject, paper.description],
          path: `/paper/${paper.id}`,
  ...
  const staticResults = searchResourcesByQuery(query);
  const normalizedQuery = query.trim().toLowerCase();
  const matchingPaperResults = paperResults.filter((paper) =>
    !normalizedQuery || [paper.title, paper.description, ...paper.keywords]
      .join(" ")
      .toLowerCase()
                  ...
                  (result: PaperSearchResult) => (
                    ...
                    >
                      ...
                      <div className="search-result-footer">
                        ...
                        <span>
                          {result.fileUrl ? "View and download paper" : "Open resource"}
                        </span>
