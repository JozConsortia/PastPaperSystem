...
function SubjectPapers() {
  ...
  const [selectedType, setSelectedType] =
    useState("all");
  const [databasePapers, setDatabasePapers] =
    useState<typeof pastPapers>([]);
  ...
  useEffect(() => {
    if (!gradeNumber || !subjectId) return;

    apiRequest<Array<typeof pastPapers[number]>>(
      `/api/papers?grade=${encodeURIComponent(gradeNumber)}&subject=${encodeURIComponent(subjectId)}`,
    )
      .then((papers) => setDatabasePapers(papers))
      .catch(() => setDatabasePapers([]));
  ...
  const basePapers = useMemo(() => {
    const allPapers = [...pastPapers, ...databasePapers];
    const uniquePapers = allPapers.filter(
      (paper, index, papers) =>
        papers.findIndex((item) => item.id === paper.id) === index,
    );

    return uniquePapers.filter(
      (paper) =>
        paper.grade === gradeNumber &&
        paper.subject === subjectId
    );
  }, [gradeNumber, subjectId, databasePapers]);
